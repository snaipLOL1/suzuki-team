...
runin() {
	...
	{ "$@" 2>>../suzuki-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../suzuki-install.log
	done
}
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" suzuki-install.log
fi

if [ -d "Suzuki/suzuki" ]; then
	cd Suzuki || {
...
if [ -f ".setup_complete" ]; then
	# If suzuki is already installed by this script
	PYVER=""
...
echo "Installing..." >suzuki-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/suzuki-team/Suzuki/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Suzuki
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/suzuki-team/Suzuki/ || {
